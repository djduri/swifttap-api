...
internal sealed class GetSampledCardVisitGeoStatisticsByCountryHandler : IQueryHandler<GetSampledCardVisitGeoStatisticsByCountryQuery, IEnumerable<CountryVisitStatisticsDTO>>
{
    private readonly DatabaseContext _dbContext;
    private readonly IUserService _userService;
    private readonly ILogger<GetSampledCardVisitGeoStatisticsByCountryHandler> _logger;

    public GetSampledCardVisitGeoStatisticsByCountryHandler(DatabaseContext dbContext,
                                                            IUserService userService,
                                                            ILogger<GetSampledCardVisitGeoStatisticsByCountryHandler> logger)
    {
    ...
    public async Task<IEnumerable<CountryVisitStatisticsDTO>> Handle(GetSampledCardVisitGeoStatisticsByCountryQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            // Brak CardId – tylko admin może zobaczyć wszystkie karty
        ...
        if (!entries.Any())
            return Enumerable.Empty<CountryVisitStatisticsDTO>();

        // Wyciągamy listę unikalnych krajów
        var countries = entries.Select(x => x.CountryCode).Distinct().ToList();
        var result = new List<CountryVisitStatisticsDTO>();

        foreach (var country in countries)
        {
            // Pobranie wszystkich wpisów dla konkretnego kraju
            var countryEntries = entries.Where(e => e.CountryCode == country).ToList();

            var countryDto = new CountryVisitStatisticsDTO
            {
                Country = country,
            ...
            var grouped = request.SampleType switch
            {
                ...
                SampleType.Month => countryEntries
            ...
            countryDto.Visits.AddRange(grouped.OrderBy(g => g.Period));
